...
# Cache live symbols from CompanyList to avoid stale stockmap rejections
LIVE_SYMBOL_CACHE_TTL_SECONDS = 300
_live_symbol_cache: Set[str] = set()
_live_symbol_cache_expires_at: float = 0.0
...
async def _get_live_company_symbols(force_refresh: bool = False) -> Set[str]:
    """Return company symbols from live CompanyList with a short TTL cache."""
    global _live_symbol_cache, _live_symbol_cache_expires_at

    now = time.time()
    if not force_refresh and _live_symbol_cache and now < _live_symbol_cache_expires_at:
        return _live_symbol_cache

    try:
        companies = await nepseAsync.getCompanyList()
        symbols = {
        ...
        if symbols:
            _live_symbol_cache = symbols
            _live_symbol_cache_expires_at = now + LIVE_SYMBOL_CACHE_TTL_SECONDS
        return _live_symbol_cache
    except Exception as exc:
        logger.warning("Live symbol refresh failed, using cached validation set: %s", exc)
        return _live_symbol_cache


async def validate_stock_or_raise(symbol: str) -> str:
    ...
    # Fallback: accept symbols that exist in live CompanyList even if stockmap is stale.
    live_symbols = await _get_live_company_symbols()
    if normalized_symbol and normalized_symbol in live_symbols:
        logger.info("Accepted symbol %s from live CompanyList fallback", normalized_symbol)
        return normalized_symbol

    # One forced refresh handles newly listed symbols that appeared after cache warm-up.
    live_symbols = await _get_live_company_symbols(force_refresh=True)
    if normalized_symbol and normalized_symbol in live_symbols:
        logger.info("Accepted symbol %s after forced live symbol refresh", normalized_symbol)
        return normalized_symbol
...
async def get_market_depth(symbol: str):
    validated_symbol = await validate_stock_or_raise(symbol)
