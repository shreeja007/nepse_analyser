    ...
    try:
        async for message in websocket:
            try:
                ...
                err_type = type(route_error).__name__
                err_text = str(route_error)
                if not err_text:
                    err_text = repr(route_error) or f"{err_type} (no message)"
                logger.exception(
                    "WebSocket route error (route=%s messageId=%s)",
                    locals().get("route"),
...
async def start_ws_server():
    server = await websockets.serve(
        ...
        "0.0.0.0",
        5555,
        max_size=10485760  # 10 MB instead of default 1 MB
    )
    print("WebSocket server started on ws://0.0.0.0:5555")
    print("Max message size: 10 MB")
